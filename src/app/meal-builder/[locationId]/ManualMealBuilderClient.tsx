...
export default function ManualMealBuilderClient({ locationId, initialMenuItemId, resources, isDemo }: { locationId: string; initialMenuItemId?: string; resources: MealBuildResources; isDemo: boolean }) {
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-7 sm:py-12">
      <Link href={`/locations/${locationId}`} className="text-sm font-bold text-emerald-800">← {resources.location?.shortName ?? resources.location?.name}</Link>
      <header className="mt-5"><p className="brand-kicker">Falcon Fuel</p><h1 className="mt-4 text-4xl font-bold tracking-[-0.04em] sm:text-5xl">Build my meal</h1><p className="mt-2 subtle">Already know what you’re eating? Add it here. Falcon Fuel will total the meal and keep your day up to date.</p></header>
      <AppNav />
      <Link href={`/meal-builder/${locationId}`} className="mt-4 inline-flex text-sm font-bold text-emerald-800">Want help deciding instead? Get a recommendation →</Link>
      ...
      {savedHistoryId && build.items.length > 0 && <aside className="surface sticky top-2 z-10 mt-5 max-h-[40vh] overflow-y-auto p-3 backdrop-blur" aria-label="Your saved meal order reference"><h2 className="eyebrow">Your order · {orderReference.locationName}</h2><ol className="mt-2 space-y-2">{orderReference.lines.map((line) => <li key={line.lineId} className="border-t border-black/[.05] pt-2 first:border-0 first:pt-0"><p className="text-[10px] font-bold uppercase tracking-wide subtle">{line.stationName}</p><p className="text-sm font-bold">{line.itemName} ×{line.quantity}</p>{line.components.length > 0 && <p className="mt-1 text-xs subtle">{line.components.map((component) => `${component.name}${component.quantity > 1 ? ` ×${component.quantity}` : ""}`).join(" · ")}</p>}</li>)}</ol></aside>}

      <section className="surface mt-6 p-5" aria-labelledby="manual-meal-heading">
        ...
        {computed.nutrition && <><dl className="mt-5 grid grid-cols-4 gap-2 border-t border-black/[.06] pt-5 text-center">{[["Calories", computed.nutrition.calories, "cal"], ["Protein", computed.nutrition.protein, "g"], ["Carbs", computed.nutrition.carbs, "g"], ["Fat", computed.nutrition.fat, "g"]].map(([label, value, unit]) => <div key={label} className="rounded-xl bg-emerald-50/70 p-2.5"><dt className="text-[10px] text-emerald-900/55">{label}</dt><dd className="mt-1 font-bold text-emerald-950">{value}{unit}</dd></div>)}</dl><details className="mt-3 rounded-xl bg-black/[.025] p-3"><summary className="cursor-pointer text-xs font-bold">ⓘ About these nutrition numbers</summary><p className="mt-2 text-xs leading-relaxed subtle">Nutrition information is based on Bentley Dining/Chartwells menu data when available and standardized serving estimates where exact portions are not published. Actual portions and preparation may vary.</p></details></>}
        ...
        <button type="button" className="primary mt-5 w-full" disabled={!computed.isValid || build.items.length === 0} onClick={saveMeal}>{savedHistoryId ? "Meal saved" : "Save this meal"}</button>
        {savedHistoryId && <div className="mt-4 border-t border-black/[.06] pt-4">{completionFraction !== undefined ? <div className="flex items-center justify-between gap-3 text-sm"><p><strong>Finished:</strong> {completionLabel(completionFraction)}</p><button type="button" className="font-bold text-emerald-800 underline" onClick={() => setShowCompletionCheckIn(true)}>Change</button></div> : <button type="button" className="text-sm font-bold text-emerald-800 underline" onClick={() => setShowCompletionCheckIn(true)}>Finished eating? Add a quick check-in</button>}{showCompletionCheckIn && <div className="surface-soft mt-3 p-4"><p className="text-sm font-bold">How much did you finish?</p><div className="mt-2 flex flex-wrap gap-2">{MEAL_COMPLETION_CHOICES.map((choice) => <button key={choice.label} type="button" className="chip" onClick={() => saveCompletion(choice.fraction)}>{choice.label}</button>)}</div><p className="mt-2 text-xs subtle">This updates today’s consumed nutrition so later recommendations can use what actually remains.</p></div>}</div>}
      </section>
