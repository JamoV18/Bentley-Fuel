...
export default function MealBuilderClient({ fallbackBuild, resources, isDemo }: { fallbackBuild: MealBuild; resources: MealBuildResources; isDemo: boolean }) {
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-7 sm:py-12">
      <Link href={`/locations/${build.locationId}`} className="text-sm font-bold text-emerald-800">← {resources.location?.shortName ?? resources.location?.name}</Link>
      <header className="mt-5">
        ...
        {personalized && <p className="mt-3 subtle">Balanced around your goals, nutrition remaining today, dietary needs, and recent variety. This recommendation only compares foods from {resources.location?.shortName ?? resources.location?.name ?? "the dining location you chose"}.</p>}
