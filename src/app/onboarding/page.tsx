...
const WEIGHT_LOSS_INTENSITIES: Array<{ value: WeightLossIntensity; title: string; description: string; warning?: string }> = [
  ...
  { value: "optimal", title: "Optimal", description: "About 20% below estimated maintenance. Falcon Fuel's stronger balanced option." },
...
const BREAKFAST_CHOICES: Array<{ value: BreakfastPreference; label: string }> = [
  { value: "eggs", label: "Eggs" },
  { value: "omelette", label: "Omelette" },
  { value: "yogurt", label: "Greek yogurt / yogurt" },
  { value: "oatmeal", label: "Oatmeal" },
  { value: "smoothie-fruit", label: "Smoothie / fruit" },
  { value: "cereal-granola", label: "Cereal / granola" },
  { value: "breakfast-sandwich", label: "Breakfast sandwich" },
  { value: "pancakes-waffles", label: "Pancakes / waffles" },
  { value: "variety", label: "I like variety" },
];
...
type Form = { goals: PrimaryGoal[]; goalDescription: string; behavioralGoals: BehavioralGoal[]; allergens: Allergen[]; diets: DietaryTag[]; breakfastPreferences: BreakfastPreference[]; unitSystem: UnitSystem; age: string; feet: string; inches: string; pounds: string; centimeters: string; kilograms: string; targetWeight: string; weightLossIntensity: "" | WeightLossIntensity; sex: "" | Sex; activity: "" | ActivityLevel };
const EMPTY: Form = { goals: [], goalDescription: "", behavioralGoals: [], allergens: [], diets: [], breakfastPreferences: [], unitSystem: "us", age: "", feet: "", inches: "", pounds: "", centimeters: "", kilograms: "", targetWeight: "", weightLossIntensity: "", sex: "", activity: "" };
...
export default function OnboardingPage() {
  ...
  const toggleBreakfast = (preference: BreakfastPreference) => {
    setError("");
    setForm((old) => {
      if (preference === "variety") {
        return { ...old, breakfastPreferences: old.breakfastPreferences.includes("variety") ? [] : ["variety"] };
      }
      const current = old.breakfastPreferences.filter((item) => item !== "variety");
      if (current.includes(preference)) return { ...old, breakfastPreferences: current.filter((item) => item !== preference) };
      if (current.length >= 4) {
        queueMicrotask(() => setError("Choose up to 4 breakfast staples, or select I like variety."));
        return old;
      }
      return { ...old, breakfastPreferences: [...current, preference] };
    });
  };
  ...
  const breakfastSummary = form.breakfastPreferences.length > 0
    ? form.breakfastPreferences.map((value) => BREAKFAST_CHOICES.find((choice) => choice.value === value)?.label ?? label(value)).join(" · ")
    : "No routine selected — Falcon Fuel will use conservative breakfast defaults.";

  return (
    <main className="mx-auto w-full max-w-5xl flex-1 px-6 py-8 sm:py-12">
      <header><Link href="/" className="brand-kicker">Falcon Fuel</Link><div className="mt-7 flex items-center justify-between"><p className="eyebrow">Step {step} of 4</p><p className="text-xs font-semibold subtle">About one minute</p></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-black/[.07]"><div className="h-full rounded-full bg-emerald-700 transition-all" style={{ width: `${step * 25}%` }} /></div></header>
