...
import { centimetersToFeetAndInches, parseBodyInput, parseTargetInput } from "@/lib/onboardingValidation";
...
const activityLabels: Record<ActivityLevel, string> = { inactive: "Inactive", "low-active": "Low active", active: "Active", "very-active": "Very active" };
...
export default function OnboardingPage() {
  ...
  function finish() {
    ...
    const targets = parseTargetInput(form);
    if (targets.error || !targets.value) return setError(targets.error ?? "Check your daily targets.");
    if (!form.goal) return;
    const profile = createUserProfile({ primaryGoal: form.goal, allergensToAvoid: form.allergens, dietaryPreferences: form.diets, metrics: currentBody.value,
      dailyTargets: targets.value }, existing);
