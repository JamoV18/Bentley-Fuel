...
const formatWeight = (kg: number, units: UserProfile["unitSystem"]) =>
  units === "metric" ? `${Math.round(kg * 10) / 10} kg` : `${Math.round(kg / 0.45359237)} lb`;

function mealName(entry: MealHistoryEntry, itemNames: Record<string, string>) {
  return entry.build.items.map((item) => itemNames[item.menuItemId] ?? "Meal item").join(" + ");
}
...
export default function TodayClient({
  ...
  const snapshot = useMemo(
    () => createDailyNutritionSnapshot(todayEntries, plan?.activeTargets ?? profile?.dailyTargets),
    [todayEntries, plan?.activeTargets, profile?.dailyTargets],
  );
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-7 pb-12 sm:py-12">
      <header>
        ...
        <h1 className="mt-3 text-4xl font-bold tracking-tight">Today</h1>
        <p className="mt-2 text-black/60">A simple view of what you have confirmed eating and what remains.</p>
      </header>
      <AppNav />
      ...
      <section className="mt-6 rounded-3xl bg-emerald-950 p-6 text-white shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm font-semibold text-white/70">{mode === "remaining" ? "Remaining today" : "Consumed today"}</p>
        ...
        <p className="mt-2 text-5xl font-bold tracking-tight">{round(headline).toLocaleString()}</p>
        <p className="text-sm text-white/65">calories</p>
        <div className="mt-6 grid grid-cols-3 gap-3 text-center">
          {[{ name: "Protein", value: values?.protein ?? snapshot.consumed.protein }, { name: "Carbs", value: values?.carbs ?? snapshot.consumed.carbs }, { name: "Fat", value: values?.fat ?? snapshot.consumed.fat }].map((macro) => (
            <div key={macro.name} className="rounded-2xl bg-white/10 p-3"><p className="text-xl font-bold">{round(macro.value)}g</p><p className="mt-1 text-xs text-white/65">{macro.name}</p></div>
          ))}
        </div>
      ...
      {pending.length > 0 && (
        <section className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          <p className="text-xs font-bold uppercase tracking-wide text-emerald-800">Quick check-in</p>
          <h2 className="mt-1 text-xl font-bold">How much did you finish?</h2>
          <p className="mt-2 text-sm text-black/60">One tap keeps today’s nutrition and your next recommendation accurate.</p>
          <div className="mt-4 space-y-4">
            {pending.map((entry) => (
              <article key={entry.id} className="rounded-xl bg-white p-4 shadow-sm">
                <p className="font-semibold">{mealName(entry, itemNames)}</p>
                <p className="mt-1 text-xs text-black/50">{locationNames[entry.locationId] ?? entry.locationId}</p>
                <div className="mt-3 flex flex-wrap gap-2">{MEAL_COMPLETION_CHOICES.map((choice) => <button key={choice.label} type="button" className="chip" onClick={() => saveCompletion(entry.id, choice.fraction)}>{choice.label}</button>)}</div>
              </article>
            ))}
          </div>
        </section>
      ...
      <section className="mt-6 rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-bold uppercase tracking-wide text-black/45">Your plan</p><h2 className="mt-1 text-xl font-bold">{plan?.phase === "maintenance" ? "Maintenance" : "Current goal"}</h2></div>
          <Link href="/profile-summary" className="text-sm font-semibold text-emerald-800 underline">View plan</Link>
        </div>
        {plan?.targetWeightKg && <p className="mt-3 text-sm"><strong>Target:</strong> {formatWeight(plan.targetWeightKg, profile.unitSystem)}</p>}
