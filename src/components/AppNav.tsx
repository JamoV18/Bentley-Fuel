  ...
  const navigation = (
    <nav className="app-nav" aria-label="Falcon Fuel app navigation">
      {items.map((item) => {
        ...
        return (
          <Link key={item.href} href={item.href} className="app-nav-item relative isolate overflow-hidden" data-active={active}>
            {active && <motion.span aria-hidden="true" className="app-nav-active-pill absolute inset-0 z-0" layoutId="app-nav-active-pill" transition={reduceMotion ? { duration: 0 } : { type: "spring", stiffness: 500, damping: 40, mass: 0.48 }} />}
            <motion.span className="app-nav-content relative z-10 inline-flex items-center justify-center" whileTap={reduceMotion ? undefined : { scale: 0.97 }} transition={reduceMotion ? { duration: 0 } : { duration: 0.12, ease: [0.22, 1, 0.36, 1] }}>
