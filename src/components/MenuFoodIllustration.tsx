...
function Leaf({ x, y, r = 0, fill = GREEN }: { x: number; y: number; r?: number; fill?: string }) {
  return <path d={`M${x-16} ${y+8}c1-20 19-31 34-19 13 11 4 31-12 34-14 3-24-7-22-17Z`} transform={`rotate(${r} ${x} ${y})`} fill={fill} stroke={INK} strokeWidth="3"/>;
}
...
function Tenders() {
  const pieces = [[112,142,-12],[154,124,7],[194,145,13],[146,164,-7]] as const;
...
function Potatoes() {
  const pieces = [[105,143,-8],[133,124,10],[161,145,-5],[192,126,11],[216,148,-9],[136,166,6],[178,164,-7]] as const;
...
function RoastedVegetables({ variant }: { variant?: string }) {
  const veg = [[105,145,RED],[132,126,ORANGE],[161,146,LIGHT_GREEN],[190,127,GREEN],[217,146,GOLD],[135,165,PURPLE],[180,165,ORANGE]] as const;
  return <g>{variant === "mushrooms" ? <Bowl/> : <Plate/>}<g stroke={INK} strokeWidth="4">{veg.map(([x,y,c],i)=><rect key={i} x={Number(x)-14} y={Number(y)-10} width="28" height="20" rx="7" fill={String(c)} transform={`rotate(${(i%3-1)*8} ${x} ${y})`}/>)}</g>{variant === "mushrooms" && <g fill={BROWN} stroke={INK} strokeWidth="3"><ellipse cx="127" cy="133" rx="18" ry="12"/><ellipse cx="166" cy="123" rx="18" ry="12"/><ellipse cx="197" cy="145" rx="18" ry="12"/></g>}</g>;
}

function GreenBeans() {
  const beans = [[105,132,18],[127,145,-12],[149,127,9],[171,146,-8],[192,126,12],[210,145,-14],[140,160,5],[184,161,-5]] as const;
...
function Pasta({ variant }: { variant?: string }) {
  const pieces = [[101,142,-13],[129,124,8],[159,143,-7],[190,126,10],[217,145,-11],[132,164,7],[169,165,-5],[201,160,8]] as const;
...
function Bread({ variant }: { variant?: string }) {
  if (variant === "garlic-knots") {
    const knots = [[112,141],[147,128],[182,142],[137,165],[178,165]] as const;
  ...
  return <g><rect x="88" y="100" width="145" height="105" rx="19" fill={dark ? "#a66e45" : "#e2b372"} stroke={INK} strokeWidth="7" transform="rotate(-4 160 152)"/>{variant === "multigrain" && <g fill={DARK_BROWN}>{[[120,125],[147,112],[181,128],[205,150],[138,169],[178,177]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="4"/>)}</g>}</g>;
}

function Meatballs() {
  const balls = [[115,141],[150,124],[187,141],[141,166],[180,166]] as const;
...
function Carrots({ variant }: { variant?: string }) {
  const carrots = [[111,143,-15],[139,126,8],[165,145,-7],[192,127,12],[216,147,-12],[144,164,5],[183,163,-5]] as const;
...
function Dumplings() {
  const dumplings = [[119,142,-12],[160,127,4],[199,144,12],[147,166,-5],[184,165,7]] as const;
...
function Fries() {
  const fries = [[103,153,-18],[123,135,13],[145,151,-7],[164,129,10],[185,149,-11],[207,135,15],[132,168,7],[173,169,-5],[213,160,-14]] as const;
...
function SlicedProtein({ variant }: { variant?: string }) {
  if (variant === "pepperoni") return <g fill={RED} stroke={INK} strokeWidth="4">{[[120,128],[158,118],[190,136],[139,158],[177,161]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="22"/>)}</g>;
  return <g fill="#efc1ae" stroke={INK} strokeWidth="4">{[[115,122,-8],[143,139,5],[174,119,-6],[198,145,7]].map(([x,y,r],i)=><path key={i} d={`M${x-34} ${y}q34-22 68 0q-34 20-68 0Z`} transform={`rotate(${r} ${x} ${y})`}/>)}</g>;
}

function Ingredient({ variant }: { variant?: string }) {
  if (variant === "spinach" || variant === "romaine" || variant === "iceberg") return <g>{[[112,139,-18],[142,119,8],[171,139,-6],[200,122,15],[135,163,8],[181,163,-9]].map(([x,y,r],i)=><Leaf key={i} x={x} y={y} r={r} fill={variant === "iceberg" ? LIGHT_GREEN : i%2?GREEN:DARK_GREEN}/>)}</g>;
  ...
  if (variant === "cherry-tomatoes") return <g fill={RED} stroke={INK} strokeWidth="3">{[[115,135],[145,120],[176,137],[204,123],[139,159],[183,160]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="15"/>)}</g>;
  if (variant === "red-onion") return <g fill="none" stroke={PURPLE} strokeWidth="12">{[[130,139,35],[183,145,31]].map(([x,y,r],i)=><circle key={i} cx={x} cy={y} r={r}/>)}</g>;
  if (variant === "pickle" || variant === "cucumber" || variant === "jalapeno") return <g fill={variant === "pickle" ? "#8eae3a" : variant === "jalapeno" ? GREEN : "#9fd45d"} stroke={INK} strokeWidth="4">{[[122,135],[160,122],[196,142],[146,160],[187,162]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="20"/>)}</g>;
  if (variant === "red-pepper" || variant === "mixed-peppers") return <g fill="none" strokeWidth="13" strokeLinecap="round">{[[110,128,RED],[145,151,GREEN],[180,126,RED],[205,154,GREEN]].map(([x,y,c],i)=><path key={i} d={`M${x-22} ${y}q22-18 44 0`} stroke={c}/>)}</g>;
  if (variant === "giardiniera") return <RoastedVegetables variant="mixed"/>;
  if (variant === "shredded-carrots") return <g stroke={ORANGE} strokeWidth="9" strokeLinecap="round">{[[110,130,45],[130,145,-40],[155,127,35],[177,147,-35],[201,132,45],[145,163,15]].map(([x,y,r],i)=><line key={i} x1={x-18} y1={y} x2={x+18} y2={y} transform={`rotate(${r} ${x} ${y})`}/>)}</g>;
  return <g fill={LIGHT_GREEN} stroke={INK} strokeWidth="4">{[[116,137],[149,122],[183,140],[139,162],[177,164]].map(([x,y],i)=><rect key={i} x={x-15} y={y-11} width="30" height="22" rx="6"/>)}</g>;
}
...
function Spread({ variant }: { variant?: string }) {
  ...
  return <g><Bowl/><ellipse cx="160" cy="127" rx="68" ry="27" fill={fill} stroke={INK} strokeWidth="4"/>{variant === "chickpea-salad" && <g fill={GREEN} stroke={INK} strokeWidth="2"><circle cx="134" cy="125" r="7"/><circle cx="174" cy="137" r="7"/></g>}{variant === "garbanzo" && <g fill={ORANGE} stroke={INK} strokeWidth="2">{[[125,125],[151,137],[177,123],[198,143],[141,151]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="7"/>)}</g>}</g>;
}

function Cheese({ variant }: { variant?: string }) {
  if (variant === "grated-parmesan") return <g fill="#f0db93" stroke={INK} strokeWidth="2">{[[118,143],[136,128],[155,145],[176,127],[196,146],[146,163],[181,162]].map(([x,y],i)=><rect key={i} x={x-9} y={y-4} width="18" height="8" rx="3" transform={`rotate(${i%2?18:-18} ${x} ${y})`}/>)}</g>;
...
function Chips({ variant }: { variant?: string }) {
  const pieces = [[102,145,-18],[128,124,12],[156,145,-7],[185,125,14],[215,145,-11],[137,165,8],[178,165,-7]] as const;
...
function Grain({ variant }: { variant?: string }) {
  ...
  return <g><Bowl/><ellipse cx="160" cy="128" rx="67" ry="27" fill={fill} stroke={INK} strokeWidth="4"/><g fill={CREAM}>{[[125,124],[146,136],[170,121],[194,137],[213,122],[136,151],[181,151]].map(([x,y],i)=><ellipse key={i} cx={x} cy={y} rx="6" ry="3"/>)}</g></g>;
}

function SaladProtein({ variant }: { variant?: string }) {
  if (variant === "eggs") return <g fill={CREAM} stroke={INK} strokeWidth="4">{[[120,137],[151,121],[184,138],[144,160],[183,161]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="18"/>)}</g>;
  if (variant === "tofu") return <g fill="#ead4a5" stroke={INK} strokeWidth="4">{[[119,137],[151,121],[184,139],[143,162],[181,162]].map(([x,y],i)=><rect key={i} x={x-16} y={y-13} width="32" height="26" rx="5"/>)}</g>;
...
function Topping({ variant }: { variant?: string }) {
  ...
  if (variant === "fried-onions" || variant === "chow-mein") return <g fill="none" stroke={fill} strokeWidth="9" strokeLinecap="round">{[[110,133,24],[137,150,-28],[165,128,17],[193,150,-20],[211,130,28],[154,166,8]].map(([x,y,r],i)=><path key={i} d={`M${x-18} ${y}q18-14 36 0`} transform={`rotate(${r} ${x} ${y})`}/>)}</g>;
  return <g fill={fill} stroke={INK} strokeWidth="3">{[[118,137],[146,121],[175,139],[202,124],[141,161],[185,160]].map(([x,y],i)=><rect key={i} x={x-10} y={y-7} width="20" height="14" rx="4" transform={`rotate(${i%2?12:-12} ${x} ${y})`}/>)}</g>;
}

function Dessert({ variant }: { variant?: string }) {
  if (variant === "cookie") return <g><Plate/><g fill="#7b4b33" stroke={INK} strokeWidth="6"><circle cx="132" cy="148" r="45"/><circle cx="191" cy="153" r="42"/></g><g fill="#3b281f">{[[118,132],[144,158],[181,136],[203,163],[155,122],[211,139]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="5"/>)}</g></g>;
  if (variant === "cocoa-treat") return <g><Plate/><rect x="101" y="111" width="119" height="89" rx="12" fill="#775039" stroke={INK} strokeWidth="7"/><g fill="#4b3125">{[[117,131],[143,121],[168,140],[196,125],[128,165],[158,174],[190,159]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="8"/>)}</g></g>;
...
function Soup() {
  return <g><Bowl/><ellipse cx="160" cy="127" rx="68" ry="27" fill="#f1cc5f" stroke={INK} strokeWidth="4"/><g stroke={INK} strokeWidth="2">{[[126,123,GREEN],[151,137,ORANGE],[178,121,GREEN],[202,138,RED],[137,148,LIGHT_GREEN],[185,150,ORANGE]].map(([x,y,c],i)=><circle key={i} cx={x} cy={y} r="7" fill={String(c)}/>)}</g></g>;
}

function Crackers() {
  return <g fill="#efd69b" stroke={INK} strokeWidth="5"><rect x="93" y="104" width="88" height="113" rx="10" transform="rotate(-8 137 160)"/><rect x="155" y="110" width="88" height="113" rx="10" transform="rotate(8 199 166)"/><g fill={INK} stroke="none">{[[116,135],[145,148],[124,179],[185,139],[209,160],[190,192]].map(([x,y],i)=><circle key={i} cx={x} cy={y} r="3"/>)}</g></g>;
}

function Salad() {
  return <g><Bowl/><g>{[[116,133,-18],[144,117,8],[174,132,-7],[202,119,16],[139,151,7],[182,151,-10]].map(([x,y,r],i)=><Leaf key={i} x={x} y={y} r={r} fill={i%2?GREEN:LIGHT_GREEN}/>)}</g><g fill={RED} stroke={INK} strokeWidth="2"><circle cx="130" cy="137" r="8"/><circle cx="193" cy="141" r="8"/></g></g>;
}

function PlateMix() {
  return <g><Plate/><ProteinPlate/><g transform="translate(40 48) scale(.5)"><RoastedVegetables variant="mixed"/></g></g>;
}
...
export default function MenuFoodIllustration({ name }: { name: string }) {
  ...
  switch (spec.kind) {
    case "pizza": art = <Pizza variant={spec.variant}/>; break;
    case "protein-plate": art = <ProteinPlate variant={spec.variant}/>; break;
    case "tenders": art = <Tenders/>; break;
    case "potatoes": art = <Potatoes/>; break;
    case "roasted-vegetables": art = <RoastedVegetables variant={spec.variant}/>; break;
    case "green-beans": art = <GreenBeans/>; break;
    case "pasta": art = <Pasta variant={spec.variant}/>; break;
    case "sauce": art = <Sauce variant={spec.variant}/>; break;
    case "bread": art = <Bread variant={spec.variant}/>; break;
    case "meatballs": art = <Meatballs/>; break;
    case "carrots": art = <Carrots variant={spec.variant}/>; break;
    case "dumplings": art = <Dumplings/>; break;
    case "sandwich": art = <Sandwich/>; break;
    case "burger": art = <Sandwich burger/>; break;
    case "hotdog": art = <HotDog/>; break;
    case "fries": art = <Fries/>; break;
    case "sliced-protein": art = <SlicedProtein variant={spec.variant}/>; break;
    case "ingredient": art = <Ingredient variant={spec.variant}/>; break;
    case "spread": art = <Spread variant={spec.variant}/>; break;
    case "cheese": art = <Cheese variant={spec.variant}/>; break;
    case "dressing": art = <Dressing variant={spec.variant}/>; break;
    case "chips": art = <Chips variant={spec.variant}/>; break;
    case "grain": art = <Grain variant={spec.variant}/>; break;
    case "salad-protein": art = <SaladProtein variant={spec.variant}/>; break;
    case "oil": art = <Bottle fill="#9bad31"/>; break;
    case "vinegar": art = <Bottle fill={spec.variant === "red-wine" ? "#a43d3f" : "#684436"}/>; break;
    case "topping": art = <Topping variant={spec.variant}/>; break;
    case "dessert": art = <Dessert variant={spec.variant}/>; break;
    case "soup": art = <Soup/>; break;
    case "crackers": art = <Crackers/>; break;
    case "salad": art = <Salad/>; break;
    default: art = <PlateMix/>; break;
  }
  return <svg className="ff-food-art" viewBox="0 0 320 240" aria-hidden="true" focusable="false">{art}</svg>;
}
