...
import { bentleyMenuDate, normalizeActiveBentleyMenuDate } from "./bentleyDiningDate";
...
test("live menu URLs roll past dates forward to Bentley today", () => {
  ...
  assert.equal(normalizeActiveBentleyMenuDate("2026-09-01", now), "2026-09-01");
});

test("future live menu dates remain available for intentional browsing", () => {
  const now = new Date("2026-09-01T21:30:00.000Z");
  assert.equal(normalizeActiveBentleyMenuDate("2026-09-02", now), "2026-09-02");
  assert.equal(normalizeActiveBentleyMenuDate("2026-09-15", now), "2026-09-15");
});

test("invalid live menu dates fall back to Bentley today", () => {
  const now = new Date("2026-09-01T21:30:00.000Z");
  assert.equal(normalizeActiveBentleyMenuDate(undefined, now), "2026-09-01");
  assert.equal(normalizeActiveBentleyMenuDate("not-a-date", now), "2026-09-01");
  assert.equal(normalizeActiveBentleyMenuDate("2026-02-31", now), "2026-09-01");
});
